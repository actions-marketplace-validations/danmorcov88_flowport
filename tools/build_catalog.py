    ...
    for ext in root.findall("extensions/extension"):
        ...
        for dyn in ext.findall("dynamicProperties/dynamicProperty"):
            ...
            if dynamic_scope is None or scope != "NONE":
                dynamic_scope = scope
        relationships = [
            r.findtext("name", default="") for r in ext.findall("relationships/relationship")
        ]
        types.append(
            ExtensionType(
                name=name,
...
def build_properties(release: Release) -> dict[str, object]:
    ...
    for name in sorted(release.types):
        ...
        if ext.relationships:
            entry["relationships"] = list(ext.relationships)
        types[name] = entry
    return {
...
# type supports them; "relationships" lists the relationships of processors.
#
"""
...
def main() -> int:
    ...
    for release in (source, target):
        properties_output = output.parent / "properties" / f"{release.version}.yaml"
        properties_output.parent.mkdir(parents=True, exist_ok=True)
        with properties_output.open("w", encoding="utf-8", newline="\n") as fh:
            fh.write(PROPERTIES_HEADER.format(version=release.version))
            yaml.safe_dump(
                build_properties(release), fh, sort_keys=False, allow_unicode=True, width=100
            )
